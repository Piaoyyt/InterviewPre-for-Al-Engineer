...
- One-stage
  ...
  > 网络结构采用了CSPDarkNet53+Neck（SPPNet+PAN）+yolov3head；框的回归采用了CIOU； 加入了很多的训练技巧：
   ...
   3.正则化：DropPath、DropBlock；  
     ...
     CSP结构：主要由CBL(卷积、BN层、Leaky ReLu)和其它卷积构成，并且引入了residual操作，即一端接入CBL，另一端接入普通的卷积。
     PAN结构:路径聚合网络结构，即在FPN里面增加了自下而上的结构。
   4.框的位置损失：CIOU Loss(只可以在)；框的类别损失：BCE Loss；框的置信度损失：BCE Loss。
  #### 6.Yolov5
