...
### 1.3主要流程
![pipelineRS](pics/pipelineRS.png)
1. 物料库
...
### 2.2基于内容的召回
> 基于内容的召回（ CB召回 ），一般也叫做标签召回。这类召回的核心思想是基于item自身的属性，这些属性
> 可以表达为tag，Cate，也可以用来表达用户ID，用户类型等，更可以通过⼀些交叉验证的⽅式，针对内容提取向量，
> 将内容表达为连续向量的方式进行召回。
### 2.3经典深度网络模型的召回（本质上也就是寻找最优特征表示）

#### 1.DNN模型
> 所谓的DNN模型其实就是利用深度卷积网络来生成用户的一种非线性映射，怎么映射呢？通过输入用户相关的一些特征比如
> 用户id、用户访问的频率、访问的时长、交互产品的类型这些能够反应用户个体特性的特征，然后让网络去学习用户的向量表示，
> 使得通过这些向量的表示能够与产品进行相似度的计算从而预测出每一个用户对不同产品的喜好程度，DNN里面其实就是采用
> 多分类的思想，即对于那些正样本和负样本分别对应的位置赋予1或者0.
- 与矩阵分解MF的本质是一样的，不过矩阵分解里面只利用到了用户和产品之间交互的信息，即利用到的特征维度比较低，因此
表示的信息不够丰富，相对于DNN来说就不够精确。
#### 2.DSSM
> DSSM是Deep Structured Semantic Model的缩写，即我们通常说的基于深度网络的语义模型,不同于一般的DNN里面
> 将待匹配的词向量或者其它语料库当作网络里面的最后的全连接层来学习，也就是我们所称为的key，这里是将query和key都
> 经过DNN来得到映射过后的向量，即key不再是嵌入在某一层里面，而都是经过深度网络学习得到的，最后再做内积。
- word hashing：不同于one-hot编码的方式，对所有词进行这样编码会导致维度灾难，计算量会变大，这里是对一个词进行
n-gram的方式得到词的编码形式，维度相比于独热的方式更小，但是冲突会变多一点。
## <a id = 'Rank'></a>3.排序模型
