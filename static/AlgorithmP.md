...
- 操作数得到最大值
![](pics/AliTest.png)  
    > 利用堆的性质，可以优化时间，即每一次入堆的时间是对数级别，python里面的库为heapq，可以利用其得到堆的结构。
...
- 放货物  
![](pics/PutBox.jpg)  
    > 该题就是单调栈的典型代表，为什么能用单调栈呢，因为这类题都是必须满足一个条件才会对最终结果有影响，那就是左右两边比中间的高，
和接雨水那一题一样，用单调栈去存储满足单调性的位置，然后碰到不满足的时候就说明会出现影响结果的情况，这时候直接对当前的情况进行一个
统计即可。
...
- 校验数
![](pics/CheckDigit.png)  
    > 首先利用位与运算确定数字的最大位数；然后再根据校验数的特性去找符合的数即可。
...
来构建树的代码如下：
    ```python
      ...
      while queue:
          ...
          index += 1   
    ```
    > 构造完之后，如果要判断子树相同，就可以利用递归去得到每一个结点为根的子树的结构，比较即可。
- 可能的错题排列
    > 典型的回溯问题，一般回溯问题的话都比较暴力，相当于就是在递归里面加入了一个回溯的过程，这类题目的关键就是要
弄清遍历的范围、以及当前的选择对遍历范围的影响、以及所有的可能选择。
### 2.3分治
### 2.4DFS和BFS
...
- 按图找最近的路
    ...
    考虑中转的情况。
- 智慧停车
    > 首先通过BFS找到最短的距离，并在搜寻的过程中会给每一个位置记录距离值；然后从目标点往外dfs直到找到源点即为一条路径。
### 2.5贪心
